...
import { FETCH_DATA_STEP, saveDataStep } from '../actions/updateStep';
...
const updateStep = (store) => (next) => (action) => {
  switch (action.type) {
    case FETCH_DATA_STEP: {
      ...
      axios.get('http://34.239.44.174/api/travel/53/step/155', { headers: { Authorization: `Bearer ${token}` } })
        .then((response) => {
          ...
          const dataStep = {
            title, description, latitude, longitude, pictures, stepDate, id,
          };
